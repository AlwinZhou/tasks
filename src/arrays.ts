...
export function bookEndList(numbers: number[]): number[] {
    ...
    if (numbers.length === 1) {
        ...
        // consowle.log([numbers[0], numbers[numbers.length - 1]]);
...
export function stringsToIntegers(numbers: string[]): number[] {
    ...
    // console.log(result); // Log the entire array
    return result;
}
...
export function countShortWords(words: string[]): number {
    const shortarr = words.filter((string) => string.length < 4);
...
export function allRGB(colors: string[]): boolean {
    const validcolor = ["red", "blue", "green"];
    const checkcolor = colors.every((color) => validcolor.includes(color));
    return checkcolor;
}
